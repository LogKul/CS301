...
# Assignment 6: Trees and Graphs
# Submitted by Logan Kulesus
# Due April 4th, 2022
...
class DirectedGraph:
    ...
    def search(self, search_val):
        if self.start.value == search_val:
            ...
            return self.search_recur(self.start, search_val, {self.start.id: ""})

    def search_recur(self, parent_vertex, search_val, search_catalog):
        if len(parent_vertex.children) > 0:
            for child in parent_vertex.children:
                if child.value == search_val:
                    return True
                if child.id not in search_catalog:
                    search_catalog[child.id] = ""
                    branch = self.search_recur(
                        child, search_val, search_catalog)
                    if branch == True:
                        return branch
        return False
    ...
    def vertex_dfs(self, search_id):
        if self.start.id == search_id:
            ...
            return self.vertex_dfs_recur(self.start, search_id, {self.start.id: ""})

    def vertex_dfs_recur(self, parent_vertex, search_id, search_catalog):
        if len(parent_vertex.children) > 0:
            for child in parent_vertex.children:
                if child.id == search_id:
                    return child
                if child.id not in search_catalog:
                    ...
                    branch = self.vertex_dfs_recur(
                        child, search_id, search_catalog)
                    if branch is not False:
                        return branch
        return False
...
b.put(3, [1], [])
b.put(4, [3], [2])
# This put is meant to fail to build a vertex
# because of bad input, this is why an error
# is printed.
b.put(5, [5], [6])
...
print(b.search(3))
print(b.search(4))
# We search for vertex with value 5 to
# verify that it wasn't built earlier due to
# bad input, and a vertex with a value of
# 6 doesn't exist in the graph.
print(b.search(5))
print(b.search(6))
...
#
#   a.) TREE: O(N). For the recursive DFS search for the Tree class, we have N elements that we
...
#       in O(N) time.
#
#   b.) DIRECTED GRAPH: O(N). For the recursive DFS search for the Directed Graph class, we have n
...
#       look at, we look through a collection of vertices that we have already checked
#       to determine to continue searching through that specific child or not. This could
#       take O(N) time to complete for each of our O(N) elements in the graph if we were to
#       keep track of our flagged vertices with lists, but because we are using dictionaries,
#       we have a runtime of O(1) for checking to see if a key exists in a list, which brings
#       our total runtime to O(N) * O(1) = O(N).
